...
#. Terminal can be opened from inside ``vim`` using ``,<space>``. This brings up the fish terminal, if present, or brings up bash. Quit the terminal using ``Ctrl w c`` and then ``:q``
